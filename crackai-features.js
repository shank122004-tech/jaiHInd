      ...
      try {
        ...
        const rows = members
          ...
          .map(m => {
            ...
            return { uid: m, name: memberNames[m]||'Student', msgs, qAns, act, joined, actColor };
          })
        ...
        body.innerHTML = `
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;flex-wrap:wrap;gap:6px;">
            <div>
              <div style="font-size:15px;font-weight:800;color:#fff;">${data.name||'Group'} — Dashboard</div>
              <div style="font-size:11px;color:rgba(200,195,255,0.5);">Code: <span style="font-family:monospace;color:#f59e0b;font-weight:700;letter-spacing:2px;">${inviteCode}</span></div>
            </div>
            <button class="cf-btn cf-btn-sm" onclick="CF._shareGroupCode('${inviteCode}','${(data.name||'').replace(/'/g,'')}')">📤 Share Code</button>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:14px;">
            <div style="background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.2);border-radius:10px;padding:10px;text-align:center;">
              <div style="font-size:20px;font-weight:800;color:#4ade80;">${members.length}</div>
            ...
            <div style="background:rgba(245,158,11,0.1);border:1px solid rgba(245,158,11,0.2);border-radius:10px;padding:10px;text-align:center;">
              <div style="font-size:20px;font-weight:800;color:#f59e0b;">${rows.filter(r=>r.act>0).length}</div>
            ...
            <div style="background:rgba(108,99,255,0.1);border:1px solid rgba(108,99,255,0.2);border-radius:10px;padding:10px;text-align:center;">
              <div style="font-size:20px;font-weight:800;color:#a78bfa;">${messages.length}</div>
              <div style="font-size:10px;color:rgba(200,195,255,0.5);">Messages</div>
            </div>
          </div>
          <div style="font-size:12px;font-weight:700;color:#fff;margin-bottom:6px;">📊 Student Activity</div>
          <div style="font-size:10px;color:rgba(200,195,255,0.35);margin-bottom:8px;">🔄 Auto-refreshes every 10s · Sorted by activity</div>
          ${rows.length === 0
            ? `<div style="text-align:center;padding:20px;font-size:12px;color:rgba(200,195,255,0.4);">No students yet. Share your invite code!</div>`
            : `<div style="display:flex;flex-direction:column;gap:5px;">
                <div style="display:grid;grid-template-columns:1fr 36px 36px 50px;gap:4px;padding:4px 8px;font-size:10px;color:rgba(200,195,255,0.35);font-weight:700;text-transform:uppercase;">
                  <div>Student</div><div style="text-align:center;">💬</div><div style="text-align:center;">Q's</div><div style="text-align:center;">Activity</div>
                </div>
                ${rows.map((r,i)=>`
                <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:9px;padding:9px 10px;display:grid;grid-template-columns:1fr 36px 36px 50px;gap:4px;align-items:center;">
                  <div>
                    <div style="font-size:12px;font-weight:700;color:#fff;">${i===0?'🥇 ':i===1?'🥈 ':i===2?'🥉 ':''}${r.name}</div>
                    <div style="font-size:10px;color:rgba(200,195,255,0.35);">Joined: ${r.joined}</div>
                  </div>
