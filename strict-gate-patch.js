...
(function () {
  ...
  function isPremium() {
    try {
      ...
      // Also accept legacy global key (set by older payment flows)
      if (localStorage.getItem('sscai_premium') === 'true') {
        if (typeof state !== 'undefined') state.isPremium = true;
        return true;
      }
      return false;
  ...
  window.updateLimitUI = function () {
    ...
    try {
      ...
      var rem = Math.max(0, FREE_TEXT - (typeof state !== 'undefined' ? (state.textCount || 0) : 0));
      el.textContent = rem > 0
        ? '🤖 CrackAI — ' + rem + ' free messages left today'
        : '🔒 Limit reached — Upgrade to Premium ₹199/month for unlimited access';
    } catch(e) {}
  };
